...
//        response.sendRedirect("customer?action=list"); // back to list after delete
//    }
//    
//}
...
public class CustomerController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        } else if (action.equals("view")) {   // ✅ added view action
            int accountNumber = Integer.parseInt(request.getParameter("accountNumber"));
            Customer customer = customerService.getCustomerById(accountNumber);
//            System.out.println(customer);          
            request.setAttribute("customer", customer);
            request.getRequestDispatcher("/view/customer/viewCustomer.jsp").forward(request, response);
        }
    }
